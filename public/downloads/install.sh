#!/bin/sh
# Siro - 1 command, lightweight PHP API installer (v1.0.12)
...
while [ $# -gt 0 ]; do case "$1" in
    ...
    --quiet) QUIET=true; exec >/dev/null 2>&1 ;;
    --help|-h)
        echo "Usage:"
...
SUDO=""; [ "$IS_ROOT" = "false" ] && command -v sudo >/dev/null 2>&1 && SUDO="sudo"

if command -v php >/dev/null 2>&1; then
    PHP_VER=$(php -r "echo PHP_VERSION_ID;" 2>/dev/null || echo "0")
    if [ "$PHP_VER" -ge 80200 ]; then
        log_ok "PHP $(php -r 'echo PHP_VERSION;') found"
        PHP_BIN="php"
    else
...
if [ -z "$PHP_BIN" ]; then
    ...
    case "$(uname -s)" in
        Darwin)
            ...
            brew install "php@$PHP_MAJOR" 2>/dev/null || brew upgrade "php@$PHP_MAJOR" 2>/dev/null
            brew link --overwrite "php@$PHP_MAJOR" 2>/dev/null || true
            PHP_BIN="php"
        ...
        Linux)
            if command -v apt-get >/dev/null 2>&1; then
                ...
                $SUDO apt-get install -y -qq \
                    "php$PHP_MAJOR-cli" "php$PHP_MAJOR-curl" "php$PHP_MAJOR-mbstring" \
                    "php$PHP_MAJOR-mysql" "php$PHP_MAJOR-sqlite" "php$PHP_MAJOR-xml" \
                    "php$PHP_MAJOR-sockets" "php$PHP_MAJOR-intl" \
                    2>/dev/null || \
                $SUDO apt-get install -y -qq \
                    php-cli php-curl php-mbstring php-mysql php-sqlite php-xml php-sockets php-intl \
                    2>/dev/null || log_fail "Failed to install PHP. Try: sudo apt-get install php$PHP_MAJOR-cli"
            elif command -v yum >/dev/null 2>&1; then
                $SUDO yum install -y \
                    "php-cli" "php-curl" "php-mbstring" "php-pdo" "php-mysqlnd" "php-sqlite3" "php-xml" "php-sockets" \
                    2>/dev/null || log_fail "Failed to install PHP. Try: sudo yum install php-cli php-mbstring"
            else
